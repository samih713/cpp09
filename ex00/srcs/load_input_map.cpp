 ...
 *
 * This function reads each line from the input file and parses it to extract the date and
 * value. It validates the input data and inserts it into the inputMap member variable. If
 * any errors occur during the parsing or validation process, error messages are inserted
 * into the errorMap member variable.
 *
 * @param None.
 * @return None.
 */
void BitcoinExchange::load_input_map()
{
    ...
    while (std::getline(*inputStream, line)) // get the line
    {
        ...
        if (line.length() < delimiter.length() || separator == string::npos)
            insert_error(lineNumber, "Error: bad input => " + line);
        else // validate day
        {
            try
            {
            ...
            {
                insert_error(lineNumber,
                             "Error: bad input => " + line.substr(0, separator));
            }
        }

        value =
            validate_value(lineNumber, line.substr(separator + delimiter.length()));
...
inline double BitcoinExchange::validate_value(size_t lineNumber, const string &value_str)
{
    ...
    valueStream >> value;
    if (ISERROR(lineNumber))
        return -1;
    if (!valueStream.eof() || valueStream.fail())
        insert_error(lineNumber, "Error: bad input => " + value_str);
    else if (value > 1000)
        insert_error(lineNumber, "Error: too large a number.");
    else if (value < 0)
        insert_error(lineNumber, "Error: not a positive number.");
    return value;
}
